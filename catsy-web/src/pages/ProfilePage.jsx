    ...
    return (
        <div className="min-h-screen pt-24 px-6 animate-fade-in pb-32">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-4xl font-sans font-bold text-white">My Profile</h1>

                {/* Edit Button — only shown when not editing */}
                {!isEditing && (
                    <button
                        onClick={handleEditClick}
                        className="flex items-center gap-2 text-sm font-bold bg-white hover:bg-neutral-200 text-neutral-900 px-4 py-2 rounded-full transition-colors shadow-lg"
                    >
                        <Edit2 size={16} /> Edit
                    </button>
                )}
            ...
            {/* 
              ...
              * Always map direct Supabase DB variables (snake_case e.g., 'first_name', 'last_name')
              * alongside their frontend camelCase equivalents. This ensures the UI never breaks 
              * if the global UserContext state payload structure slightly shifts!
              */}
            ...
            <div className="mb-12 space-y-6 mt-8">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        {isEditing ? (
                            <input
                                ...
                                className="w-full text-xl font-bold text-white border-b-2 border-brand-accent pb-1 outline-none bg-transparent"
                            />
                        ) : (
                            <div className="text-xl font-bold text-white border-b border-white/10 pb-2">@{userInfo.username || userInfo.user_name || 'N/A'}</div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {isEditing ? (
                            ...
                            <div className="text-xl font-bold text-white border-b border-white/10 pb-2">{userInfo.firstName || userInfo.first_name || 'N/A'}</div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {isEditing ? (
                            ...
                            <div className="text-xl font-bold text-white border-b border-white/10 pb-2">{userInfo.lastName || userInfo.last_name || 'N/A'}</div>
                        )}
                    </div>
                ...
                <div>
                    ...
                    {isEditing ? (
                        ...
                        <div className="text-xl font-bold text-white border-b border-white/10 pb-2">{userInfo.email || 'N/A'}</div>
                    )}
                </div>
                ...
                <div className="pb-4">
                    ...
                    {isEditing ? (
                        ...
                        <div className="text-xl font-bold text-white border-b border-white/10 pb-2">{userInfo.phone || userInfo.phone_number || 'N/A'}</div>
                    )}
                </div>
                    ...
                    {isEditing ? (
                        <div className="relative">
                            <input
                                ...
                                onChange={handleInputChange}
                                className="w-full text-xl font-bold text-white border-b-2 border-brand-accent pb-1 outline-none bg-transparent pr-10"
                                placeholder="Enter new password"
                            />
                            <Lock size={18} className="absolute right-0 top-1.5 text-neutral-400" />
                        </div>
                    ) : (
                        <div className="text-xl font-bold text-white border-b border-white/10 pb-2">••••••••</div>
                    )}
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 pt-1">
                                    {passwordStrength.feedback.map(req => (
                                        <div key={req.id} className="flex items-center gap-1.5">
                                            ...
                                            <span className={`text-[10px] ${req.met ? 'text-white' : 'text-neutral-400'}`}>
                                                {req.text}
                                            </span>
                                        </div>
                                    ))}
            ...
            {isEditing && (
                <div className="flex gap-4 mt-8">
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 text-sm font-bold bg-neutral-100 hover:bg-neutral-200 text-white px-6 py-3 rounded-full transition-colors"
                    >
                        <X size={16} /> Cancel
                    </button>
                    <button
                        onClick={handleSaveInitiate}
                        className="flex-1 flex items-center justify-center gap-2 text-sm font-bold bg-white hover:bg-neutral-200 text-black px-6 py-3 rounded-full transition-colors shadow-xl"
                    >
            ...
            {history.length > 0 && (
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        <div className="bg-neutral-800/50 rounded-3xl border border-neutral-100 overflow-hidden">
                            {history.map((order) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="flex justify-between items-center p-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors cursor-pointer active:bg-neutral-100"
                                    >
                    ...
                    {isLoadingReservations ? (
                        ...
                        <div className="bg-neutral-800/50 rounded-3xl border border-neutral-100 overflow-hidden">
                            {reservationHistory.map((res) => (
                                <div key={res.id} className="flex justify-between items-center p-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                                    <div className="flex flex-col">
            ...
            {showReviewModal && selectedTransaction && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 animate-fade-in">
                    <div className="bg-neutral-800/50 w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-slide-up-fade">
