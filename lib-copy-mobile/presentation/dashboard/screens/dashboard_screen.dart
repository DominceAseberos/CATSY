...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final tablesAttention = ref.watch(tablesProvider).whenData(
          (tables) => tables.where((t) => t.status.label == 'Bill Ready').length,
        );
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isRestaurantOpen ? AppColors.success : AppColors.error,
                        width: 1.5,
                      ),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          isRestaurantOpen ? Icons.wb_sunny : Icons.nightlight_round,
                          size: 16,
                          color: isRestaurantOpen ? AppColors.success : AppColors.error,
                        ),
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      TextButton(
                        onPressed: () => context.pushNamed(RouteNames.tableManagement),
                  ...
                  tablesAsync.when(
                    data: (tables) => GridView.builder(
                      ...
                      itemBuilder: (context, index) => TableTile(table: tables[index]),
                    ),
...
class _ActionRequiredCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final hasActions = pendingPayments > 0 ||
...
class _TableLegend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return tablesAsync.when(
      data: (tables) {
        final available = tables.where((t) => t.status.label == 'Available').length;
        final occupied = tables.where((t) => t.status.label == 'Occupied').length;
        final reserved = tables.where((t) => t.status.label == 'Reserved').length;
        final billReady = tables.where((t) => t.status.label == 'Bill Ready').length;

        return Wrap(
          spacing: 12,
          runSpacing: 4,
          children: [
            if (available > 0) _LegendItem(color: AppColors.success, label: 'Available ($available)'),
            if (occupied > 0) _LegendItem(color: AppColors.error, label: 'Occupied ($occupied)'),
            if (reserved > 0) _LegendItem(color: AppColors.info, label: 'Reserved ($reserved)'),
            if (billReady > 0) _LegendItem(color: AppColors.warning, label: 'Bill Ready ($billReady)'),
          ],
        );
      },
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              _NavItem(icon: Icons.receipt_long, label: 'Orders', isSelected: false, onTap: () => context.pushNamed(RouteNames.orderHistory)),
              _NavItem(icon: Icons.more_horiz, label: 'More', isSelected: false, onTap: () {}),
              _NavItem(icon: Icons.settings, label: 'Settings', isSelected: false, onTap: () {}),
            ],
          ),
        ),
      ),
